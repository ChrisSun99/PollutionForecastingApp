...
if __name__ == '__main__':
	...
	for epoch in range(epochs):
		train_loss_epoch, verify_loss_epoch = [], []
		...
		for i in range(len(train_datasets)):
			...
			if i != len(train_datasets) - 1:  # 训练集
				...
				optimizer.step()
				train_loss_epoch.append(train_loss_fn)
			elif i == len(train_datasets) - 1:  # 验证集
				y_verify_p = nn(var_x)
				y_verify_t = var_y
				verify_loss_fn = criterion(y_verify_p, y_verify_t, weights)
				verify_loss_epoch.append(verify_loss_fn)

		if (epoch + 1) % 100 == 0:  # 间隔打印训练结果
			train_loss_epoch = np.mean([p.data.cpu().numpy() for p in train_loss_epoch])
			verify_loss_epoch = np.mean([p.data.cpu().numpy() for p in verify_loss_epoch])
			train_loss_record.append([epoch + 1, train_loss_epoch])
			verify_loss_record.append([epoch + 1, verify_loss_epoch])
			print('epoch: {}, train_loss: {:6f}, verify_loss: {:6f}'.format(epoch + 1, train_loss_epoch, verify_loss_epoch))

		# 早停
		if epoch > 100 * 10:
			last_verify_losses = [p[1] for p in verify_loss_record[-10:]]
	...
	plt.plot([p[0] for p in train_loss_record[10:]], [p[1] for p in train_loss_record[10:]])
	plt.plot([p[0] for p in verify_loss_record[10:]], [p[1] for p in verify_loss_record[10:]], 'r--')
