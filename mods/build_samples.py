...
def build_data_frame_for_correlation_analysis(start_time, end_time):
    ...
    data['time_stamp'] = data.loc[:, 'ptime'].apply(
        lambda x: int(time.mktime(time.strptime(str(int(float(x))), "%Y%m%d%H"))))
    ...
    # 合并数据
    data = pd.concat([data[['ptime', 'time_stamp']], filtered_numerical_data, encoded_categorical_data], axis=1,
                     sort=False)
